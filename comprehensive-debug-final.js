...
console.log = function(...args) {
  ...
  if (typeof message === 'string') {
    ...
    if (message.includes('🔍 useEffect triggered with dependencies')) {
      ...
      console.warn(`📊 USEEFFECT TRIGGERS: ${window.mapDebug.useEffectTriggers}`);
    }
    
    // Track place additions
    if (message.includes('🔄 PlannerForm - Notifying parent of handlePlaceSelect change')) {
...
window.checkMapDebug = function() {
  ...
  if (mapContainer) {
    const markers = mapContainer.querySelectorAll('[class*="marker"], div[style*="position: absolute"]');
...
window.analyzeDataFlow = function() {
  ...
  if (stats.formCallbacks > 0 && stats.useEffectTriggers === 0) {
    console.error("❌ ISSUE: Form callbacks received but useEffect not triggered!");
    console.log("This suggests the useEffect dependencies are not working correctly.");
  }
  
  if (stats.useEffectTriggers > 0 && stats.mapUpdates === 0) {
    console.error("❌ ISSUE: useEffect triggered but Map not updated!");
    console.log("This suggests the Map component is not receiving updated props.");
  }
  
  if (stats.mapUpdates > 0) {
    console.log("✅ Map is receiving updates - check if markers are being rendered correctly.");
  }
  
  // Expected flow: Place Addition -> Form Callback -> useEffect Trigger -> Map Update
  const expectedSequence = ["Place Addition", "Form Data Callback", "useEffect Trigger", "Map Update"];
  const recentEvents = stats.timeline.slice(-10);
  
  console.log("\n🔄 EXPECTED FLOW ANALYSIS:");
  expectedSequence.forEach(step => {
    const found = recentEvents.find(event => event.event.includes(step.split(' ')[0]));
    console.log(`${found ? '✅' : '❌'} ${step}: ${found ? 'Found' : 'Missing'}`);
  });
};
...
setInterval(() => {
  const stats = window.mapDebug;
  const totalActivity = stats.formCallbacks + stats.mapUpdates + stats.placeAdditions + stats.placeRemovals;
  
  if (totalActivity > 0) {
    console.log(`⏰ Auto-check [${new Date().toLocaleTimeString()}]: Total activity: ${totalActivity}`);
  }
}, 3000);
...
console.log("🔍 The script will automatically track all events and help identify issues.");
