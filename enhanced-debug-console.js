...
console.log = function (...args) {
  ...
  if (typeof message === "string") {
    ...
    if (
      ...
      console.warn(
        "📊 STATS: Form callbacks: " + window.debugMapMarkers.formCallbacks
      );
    }
    ...
    if (message.includes("🗺️ Map component - Places data")) {
      ...
      console.warn(
        "📊 STATS: Map updates: " + window.debugMapMarkers.mapUpdates
      );
    }

    // Track place additions
    if (
      message.includes("🔄 PlannerForm - Notifying parent of form change") &&
      args[1] &&
      args[1].changeType === "addPlace"
    ) {
      window.debugMapMarkers.placeAdditions++;
      console.warn(
        "📊 STATS: Place additions: " + window.debugMapMarkers.placeAdditions
      );
    }
    ...
    if (
      ...
      console.warn(
        "📊 STATS: Place removals: " + window.debugMapMarkers.placeRemovals
      );
    }
  }
...
window.checkMapState = function () {
  ...
  if (mapContainer) {
    const markers =
      mapContainer.querySelectorAll('[class*="marker"]') ||
  ...
  return {
    ...
    visibleMarkers: mapContainer
      ? mapContainer.querySelectorAll('[class*="marker"]').length ||
        mapContainer.querySelectorAll('div[style*="position: absolute"]').length
      : 0,
  };
};
...
setInterval(() => {
  ...
  if (
    stats.formCallbacks > 0 ||
    stats.mapUpdates > 0 ||
    stats.placeAdditions > 0 ||
    stats.placeRemovals > 0
  ) {
